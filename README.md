...
<4/21 전체 작업 내용>
1. 엣지 디바이스 기기 등록 로직 동적화 및 자동화
기존 문제: mic_test.py 코드 내에 HIVE_ID가 하드코딩되어 있어, 사용자가 앱에서 벌통을 추가할 때마다 소스 코드를 직접 수정해야 하는 운영상의 한계 발생.

원인 분석: 실제 서비스 환경에서는 사용자별로 고유 ID가 다르므로, 소스 코드 수정 없이 실행 시점에 기기를 서버와 페어링하는 '프로비저닝(Provisioning)' 과정이 필요함.

해결: edge_config.json 파일을 활용한 설정 관리 로직 도입. 최초 실행 시 앱에서 생성된 ID를 입력받아 로컬에 저장하고, 이후 실행 시에는 별도 입력 없이 자동으로 서버와 연동되도록 구현하여 사용자 편의성 및 확장성 확보.

2. 파이썬 실행 환경 에러 및 런타임 버그 수정
현황: python.exe 경로 인식 오류 및 코드 실행 중 NameError, SyntaxError 발생으로 시스템 중단.

조치 사항:

환경 최적화: 윈도우 '앱 실행 별칭' 설정 해제를 통해 마이크로소프트 스토어 간섭 차단 및 파이썬 실행 경로 정상화.

변수 초기화: 루프 내 사용되는 LAST_CHECK_TIME, IS_ENABLED 등의 변수를 루프 밖에서 초기화하여 참조 오류 해결.

구문 교정: requests.post 요청 시 JSON 바디 내 누락된 콤마(,) 수정 및 timeout 설정을 통한 네트워크 불안정 상황 대비 안정성 강화.

3. Flutter FCM 알림 데이터 전달 구조 개선
이슈: 앱 종료 상태에서 알림을 통해 진입할 때 _initialMessage 전역 변수가 유효하지 않거나, Null Safety 및 Scope 문제로 인해 데이터 전달 실패.

분석: 전역 변수 방식은 파일 간 가시성(Visibility) 확보가 어렵고, 위젯 생명주기(Lifecycle)와 맞지 않아 데이터 유실 위험이 큼.

대안: 생성자 주입(Constructor Injection) 방식 채택. main.dart에서 받은 초기 메시지를 SplashScreen을 거쳐 HomeScreen까지 생성자로 직접 전달하도록 구조를 변경하여 데이터 전달의 신뢰성 확보.

4. 시스템 아키텍처 및 필터링 전략 정립 (발표용)
내용:

기술 스택 정의: FastAPI(서버 구축 도구)와 REST API(통신 규약)의 역할을 명확히 구분하여 기술적 완성도 강조.

에너지 기반 필터링: FFT(고속 푸리에 변환)를 활용해 엣지 단계에서 말벌(100~160Hz)과 꿀벌 비명(4k~10kHz) 주파수 에너지를 1차 판독.

기대 효과: 무의미한 환경 소음 전송을 차단하여 서버 부하를 줄이고 데이터 통신 비용을 절감하는 **'지능형 엣지 게이트웨이'**로서의 기능 구현.
